...
C--     x = Bjorken-x value.
C--     q = sqrt(Q2) in GeV.
C--     IPN = 1, 2 for p or n. 3 dssz. 4eps09
...
C--   G.W. 25/07/2007 Add charm and bottom contributions to singlet.
   ...
   21 CONTINUE
      ...
      AL1c=log(1.-xcmup)
      ffc=Fc+Fc*AL*CF*(-9.-2.*PI2/3.+AL1c*(-3.+2.*AL1c))
      ffc=ffc+fc*al*al*c2nn2c(xcmup,3)
...
C--   G.W. 25/07/2007 Add charm and bottom contributions to singlet.
  ...
  321 CONTINUE
      ...
      AL1b=log(1.-xbmup)
      ffb=Fb+Fb*AL*CF*(-9.-2.*PI2/3.+AL1b*(-3.+2.*AL1b))
      ffb=ffb+fb*al*al*c2nn2c(xbmup,3)
...
c$$$      singxy=ftemp(5)+9./8.*ftemp(3) ! Why 9/8 and not 9/4?
     ...
     .(fbxy*c2nn2a(ymul,3) + (fbxy-fb)*c2ns2b(ymul,3))
...
C--   Proton structure functions.
      ...
      INTEGER ISET,AAA,ZZZ,pset,IPN,isetdssz,isetmstw
c      COMMON/mstwfiles/iset,prefix,cl
      ...
      common/psetmstw/isetmstw
       ...
       cl = '68cl'
       prefix1=prefix
      IF (isetmstw.EQ.0) THEN
         prefix1 = prefix
      ELSE
         prefix1 = prefix(1:len_trim(prefix))//'.'//cl
      END IF
c      aaa=1
       XMUF = dsqrt((XLAM**2.d0)*exp(S0*exp(S)))
      

c      q2=xmuf**2.d0

      CALL GetAllPDFs(prefix1,isetmstw,x,xmuf,uv,dv,ub,db,ss,ss2,
     &     c,c2,b,b2,g,phot)


        
c        write(6,*) uv,ruvz,ruv9
c        stop
      if (IPN.EQ.1) then ! proton
      upv=uv
      dnv=dv
      usea=ub
      dsea=db
      ...
      bbar=b2
      glu=g
      else if (IPN.EQ.3) then !nucleus
      
       
      call DSSZr(X,XMUF,AAA,RUVz,RDVz,RUBz,RDBz,RSz,RCz,RBz,RGz)
      upv=(RUVz*uv*ZZZ+rdvz*dv*(AAA-ZZZ))/AAA
      dnv=(rdvz*dv*ZZZ+ruvz*uv*(AAA-ZZZ))/AAA
      usea=(rubz*ub*ZZZ+rdbz*db*(AAA-ZZZ))/AAA
      dsea=(rdbz*db*ZZZ+rubz*ub*(AAA-ZZZ))/AAA
      str=ss*rsz
      sbar=ss2*rsz
      chm=c*rcz
      cbar=c2*rcz
...
c      if (rgz.lt.0.d0) then
c      write(6,*) x,xmuf,rgz      
c      endif
      
      else if (IPN.EQ.4) then !nucleus
      
        call EPS09(2,pset,AAA,x,XMUF,ruv9,rdv9,rub9,rdb9,rs9,
     1   rc9,rb9,rg9)
     
     
      upv=(RUV9*uv*ZZZ+rdv9*dv*(AAA-ZZZ))/AAA
      dnv=(rdv9*dv*ZZZ+ruv9*uv*(AAA-ZZZ))/AAA
      usea=(rub9*ub*ZZZ+rdb9*db*(AAA-ZZZ))/AAA
      dsea=(rdb9*db*ZZZ+rub9*ub*(AAA-ZZZ))/AAA
      str=ss*rs9
      sbar=ss2*rs9
      chm=c*rc9
      cbar=c2*rc9
...
*    ln3(1-y), and ln2(1-y) are exact up to truncation.
*
      FUNCTION C2G2A (Y, NF)
      IMPLICIT DOUBLE PRECISION (A-H, O-Z)
...
c Approx version of NNLO c2g in FFNS Q2<M2
...
c Approx version of NNLO c2q in FFNS Q2<M2
...
c Approx version of NNLO clg in FFNS Q2<M2
...
c Approx version of NNLO clq in FFNS Q2<M2
...
c Simplified version of NLO l2 in FFNS Q2>M2
...
!
!
! ..Calculation of the alpha_s2 heavy-quark singlet operator matrix
!    element (OME) a2_Hg(x) in the MS(bar) scheme for mu_f = m_H via
...
!
      ...
      A2HGA = - 24.89d0 / Y - 187.8d0 + 249.6d0 * Y
     &     - 146.8d0 * DL**2 * DL1
